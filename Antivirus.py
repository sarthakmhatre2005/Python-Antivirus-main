...
from PyQt5.QtWidgets import QApplication, QMessageBox, QFileDialog, QProgressDialog, QDialog, QVBoxLayout, QPushButton, QLabel, QTableWidget, QTableWidgetItem
...
# Efficient ScanThread with extension filtering and progress
class ScanThread(QThread):
    progress = pyqtSignal(int, int)
    finished = pyqtSignal(list, int)
    status = pyqtSignal(str)

    def __init__(self, root_dir, valid_exts=None):
        QThread.__init__(self)
        self.root_dir = root_dir
        self.valid_exts = valid_exts or [
    ...
    def run(self):
        ...
        for root, _, files in os.walk(self.root_dir):
            for file in files:
                ...
                if any(path.lower().endswith(ext) for ext in self.valid_exts):
        ...
        def scan_file(path):
            ...
            try:
                ...
                c = conn.cursor()
                c.execute(
                    "SELECT malware_name, threat_level FROM signatures WHERE sha256_hash=?",
                    (h,)
                )
                res = c.fetchone()
                conn.close()
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(scan_file, p) for p in all_files]
        ...
        self.finished.emit(infected, total_files)

class ScanProgressDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Full System Scan")
        self.setModal(True)
        self.resize(420, 160)
...
def scan_full_system(self):
    folder = QFileDialog.getExistingDirectory(None, "Select Folder or Drive to Scan", "C:/")
    if not folder:
        return
    self.Tabs.setCurrentIndex(3)
    self.LoadingPageTitle.setText(f"Scanning: {folder}")
    QApplication.processEvents()

    progress_dialog = ScanProgressDialog()
    ...
    def on_progress(scanned, total):
        if total > 0:
            ...
            progress_dialog.label.setText(f"Scanned {scanned}/{total} files...")
        QApplication.processEvents()
    ...
    def on_status(msg):
        progress_dialog.label.setText(msg)

    def on_finished(infected, total):
        progress_dialog.progress_bar.setValue(100)
        progress_dialog.close()
        # Save to scan history
        conn = sqlite3.connect('scan_history.db')
        c = conn.cursor()
        c.execute('INSERT INTO history (date, folder, files_scanned, infected_count, infected_details) VALUES (?, ?, ?, ?, ?)',
                (datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), folder, total, len(infected), '\n'.join([f"{p} → {n} ({l})" for p, n, l in infected])))
        conn.commit()
        conn.close()
        if infected:
            msg = f"Scanned {total} files.\n\n" + "\n".join([f"{p} → {n} ({l})" for p, n, l in infected])
            QMessageBox.information(None, "Malware Found", msg)
        else:
            QMessageBox.information(None, "Clean", f"Scanned {total} files. No malware found.")
        self.Tabs.setCurrentIndex(0)

    self.thread = ScanThread(folder)
    self.thread.progress.connect(on_progress)
    self.thread.finished.connect(on_finished)
    self.thread.status.connect(on_status)
    progress_dialog.pause_btn.clicked.connect(self.thread.pause)
    progress_dialog.resume_btn.clicked.connect(self.thread.resume)
    progress_dialog.cancel_btn.clicked.connect(self.thread.stop)
    self.thread.start()
...
def displayResults_VIRUS(self, file):
    ...
    #full system scan 
    self.FullScanButton.clicked.connect(lambda: scan_full_system(self))
...
def scan(file, self, MainWindow):
    try:
        ...
        def is_hash_malicious(sha256_hash):
            conn = sqlite3.connect('signatures.db')
            cursor = conn.cursor()
            cursor.execute("SELECT malware_name, threat_level FROM signatures WHERE sha256_hash = ?", (sha256_hash,))
            result = cursor.fetchone()
            conn.close()
            return result  # None = clean, otherwise = malicious info
...
"image: url(:/res/SideBar/settings.svg);")
        ...
        self.FullScanButton = QtWidgets.QPushButton(self.HomeTab)
        self.FullScanButton.setGeometry(QtCore.QRect(5, 125, 121, 31))
        font = QtGui.QFont()
        font.setPointSize(11)
        self.FullScanButton.setFont(font)
        self.FullScanButton.setObjectName("FullScanButton")
        self.FullScanButton.setText("Scan Full System")
        ...
        self.ScanHistoryButton = QtWidgets.QPushButton(self.HomeTab)
        self.ScanHistoryButton.setGeometry(QtCore.QRect(5, 165, 121, 31))
        self.ScanHistoryButton.setText("Scan History")
        self.ScanHistoryButton.clicked.connect(lambda: show_scan_history(self))
        self.ScheduleScanButton = QtWidgets.QPushButton(self.HomeTab)
        self.ScheduleScanButton.setGeometry(QtCore.QRect(5, 205, 121, 31))
        self.ScheduleScanButton.setText("Schedule Scan")
        self.ScheduleScanButton.clicked.connect(lambda: schedule_scan(self))
...
class ScanHistoryDialog(QDialog):
    def _init_(self):
        ...
        self.table.setHorizontalHeaderLabels(['Date', 'Folder', 'Files Scanned', 'Infected', 'Details'])
    ...
    def load_history(self):
        conn = sqlite3.connect('scan_history.db')
        c = conn.cursor()
        c.execute('SELECT date, folder, files_scanned, infected_count, infected_details FROM history ORDER BY id DESC')
        rows = c.fetchall()
...
# --- Scan Scheduling Thread ---
class SchedulerThread(QThread):
    status = pyqtSignal(str)
    def _init_(self, scan_func):
        super()._init_()
...
# --- Add Scan History and Scheduling to UI ---
def show_scan_history(self):
    dlg = ScanHistoryDialog()
    dlg.exec_()

def schedule_scan(self):
    folder = QFileDialog.getExistingDirectory(None, "Select Folder or Drive to Schedule Scan", "C:/")
    if not folder:
        return
    # Example: schedule every day at 12:00
    schedule.clear()
    schedule.every().monday.at("08:00").do(lambda: scan_full_system(self))
    if not hasattr(self, 'scheduler_thread') or not self.scheduler_thread.isRunning():
        self.scheduler_thread = SchedulerThread(lambda: scan_full_system(self))
        self.scheduler_thread.start()
    QMessageBox.information(None, "Scheduled", f"Scan scheduled for {folder} every Monday at 10:00 AM.")
